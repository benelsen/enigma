...
function Enigma(rotors, ukw, steckerbrett, eintrittswalze) {

  if ( rotors.length === 3 && ukw && steckerbrett && eintrittswalze) {
    this.rotors = rotors;
    this.ukw = ukw;
    this.steckerbrett = steckerbrett;
    this.eintrittswalze = eintrittswalze;
  } else {
...
Enigma.prototype.signal = function(signal) {
  ...
  signal = this.eintrittswalze.signal(signal);
